...
class NewsController {
    async create(req, res) {
        try {
            ...
            const result = await NewsService_1.default.createNews(req);
    ...
    async index(req, res) {
        try {
            ...
            const result = await NewsService_1.default.getNews(page, display_per_page, sort_by, sort_order, filter_by, filter_type);
    ...
    async update(req, res) {
        try {
            ...
            const result = await NewsService_1.default.updateNews(req);
    ...
    async delete(req, res) {
        try {
            const result = await NewsService_1.default.deleteNews(req.params.id);
    ...
    async show(req, res) {
        try {
            const result = await NewsService_1.default.getNewsById(req.params.id);
            return res.status(200).json({ message: 'News fetched successfully', data: result });
        }
        catch (error) {
            return res.status(500).json({ message: error instanceof Error ? error.message : 'Unknown error' });
