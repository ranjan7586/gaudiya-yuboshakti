...
const CreateForum: React.FC<CreateForumProps> = ({ is_update = false }: CreateForumProps) => {
    ...
    useEffect(() => {
        ...
        if (is_update && id) {
            const fetchForum = async () => {
                try {
                    const { data } = await axiosAuth.get(`${import.meta.env.VITE_BACKEND_URL}/api/v1/forums/details/${id}`);
                    setFormData({
                        _id: data.data._id,
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (is_update) {
                const { data } = await axiosAuth.patch(`${import.meta.env.VITE_BACKEND_URL}/api/v1/forums/update/${updateId}`, form, {
                    headers: { "Content-Type": 'multipart/form-data' },
                });
                toast.success(data.message);
                navigate('/admin/forums');
            } else {
                const { data } = await axiosAuth.post(`${import.meta.env.VITE_BACKEND_URL}/api/v1/forums/create`, form, {
                    headers: { "Content-Type": 'multipart/form-data' },
                });
                toast.success(data.message);
                setFormData({
    ...
    const placeholderTags = ['questions', 'announcements', 'feedback', 'tech'];

    return (
        <div className="min-h-screen bg-gray-50 text-gray-800">
            <div className="flex">
                <div className="form_content p-6 flex-1">
                    <div className="flex justify-between items-center mb-6">
                        <h2 className="text-2xl font-bold">Add New Forum Post</h2>
                    </div>
                                ...
                                <div className="p-4 space-y-4">
                                    ...
                                    <div className="flex flex-col space-y-2">
                                        <button
                                            ...
                                            className={`px-6 py-2 rounded-md text-white font-medium ${isSubmitting ? 'bg-blue-400' : 'bg-blue-600 hover:bg-blue-700'} focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-all`}
                                        >
                                            {isSubmitting ? (is_update ? 'Updating...' : 'Publishing...') : (is_update ? 'Update Forum Post' : 'Publish Forum Post')}
                                        </button>
                                        <button
                                            type="button"
                                        ...
                                        <div className="border-2 border-dashed border-gray-300 rounded-md p-6 text-center hover:border-blue-500 transition-colors cursor-pointer bg-gray-50">
                                            <input
                                                ...
                                                onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
                                                    ...
                                                    if (file) {
                                                        setFormData((prev) => ({ ...prev, thumbnail_img: file }));
                                                    }
                                                }}
                                            />
                                    ...
                                    {formData.thumbnail_img && (
                                        <div className="space-y-3">
                                            <div className="w-full border border-gray-300 rounded-md overflow-hidden bg-gray-50">
                                                <img
                                                    src={typeof formData.thumbnail_img === 'string' ? formData.thumbnail_img : URL.createObjectURL(formData.thumbnail_img)}
